...
var eventLabels = new Dictionary<EventType, string>
{
    ...
    [EventType.Regency] = "начатых регентств",
    [EventType.Mercenaries] = "найма наёмников",
    [EventType.Masterwork] = "созданных шедевров",
    [EventType.Banditry] = "разбойных нападений",
    [EventType.Exile] = "изгнаний вместо казни"
};
...
void PrintFinalReport(WorldStatistics stats, double knowledge)
{
    ...
    foreach (var settlementStat in stats.Settlements)
    {
        ...
        Console.WriteLine(
            ...
            $"[{settlement.X:0}, {settlement.Y:0}, плодородие {ClimateSystem.GetFertility(settlement, world):0.00}, {TerrainSystem.GetName(TerrainSystem.GetRelief(settlement, world))}]: " +
...
void PrintKingdoms(List<Kingdom> kingdoms)
{
    ...
    foreach (var kingdom in kingdoms)
    {
        ...
        var regent = KingdomSystem.GetRegent(kingdom, world);
        var regentText = regent != null ? $", регент — {SurnameSystem.GetDisplayFullName(regent)}" : "";
        ...
        Console.WriteLine(
            ...
            $"поселений под контролем: {kingdom.Settlements.Count}{capitalText}{regentText}{taxText}{alliesText}{treasuryText}{courtText}");
    }
