...
public static class YearProcessor
{
    public static void Process(World world)
    {
        ...
        // 21. Шедевры: глава цеха на пике мастерства изредка приносит поселению
        // разовый золотой куш и легенду (после рынка — шедевр не сырьё для сбыта)
        MasterworkSystem.Process(world);

        // 22. Перемена ремесла: нужда поселения перебивает привычное занятие
        CareerSystem.Process(world);

        // 23. Миграция из голодающих поселений
        MigrationSystem.Process(world);

        // 24. Колонизация: переполненные поселения основывают новые
        ColonizationSystem.Process(world);

        // 25. Государства: обновление контроля территорий, престолонаследие, новые королевства
        KingdomSystem.Process(world);

        // 26. Столица: престол там, где государь
        CapitalSystem.Process(world);

        // 27. Двор: наследник, воевода, казначей и советник при короне
        CourtSystem.Process(world);

        // 28. Шпионаж: канцлер тайно уводит часть казны у соперника
        IntrigueSystem.Process(world);

        // 29. Союзы между государствами
        AllianceSystem.Process(world);

        // 30. Войны за спорные поселения между государствами
        WarSystem.Process(world);

        // 31. Восстания: поселения отказывают короне в повиновении
        RebellionSystem.Process(world);

        // 32. Расколы веры в общинах, отрезанных от единоверцев
        SchismSystem.Process(world);

        // 33. Накопление знаний и смена эпох
        TechnologySystem.Process(world);

        // 34. Заговоры против правителя
